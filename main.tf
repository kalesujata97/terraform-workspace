...
resource "aws_iam_role" "s3_lamda_iam_role" {
...
resource "aws_iam_role_policy_attachment" "dynamo-full-policy-attach" {
  role       = aws_iam_role.s3_lamda_iam_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonDynamoDBFullAccess"
  depends_on = [aws_iam_role.s3_lamda_iam_role]
}
...
resource "aws_lambda_function" "s3_lambda_trigger" {
  ...
  role          = aws_iam_role.s3_lamda_iam_role.arn
  handler       = "handler.py"
